...
### 2026-09-17 — CRÍTICO: RLS deshabilitado en las 13 tablas de `public` (hallazgo del linter de Supabase)

**Contexto:** el usuario compartió el reporte del linter de seguridad de Supabase, que marcaba las 13 tablas de la app (más `_prisma_migrations`) como `ERROR` de nivel `SECURITY` — expuestas vía la API PostgREST de Supabase (`https://<proyecto>.supabase.co/rest/v1/<tabla>`) sin Row Level Security habilitado.

**Por qué era explotable de verdad, no una advertencia teórica:** `NEXT_PUBLIC_SUPABASE_ANON_KEY` es pública por diseño — viaja en el bundle de JS del navegador, cualquiera puede extraerla. La autorización real de esta app (`requireWorkspace()`) vive **solo** adentro del código de Next.js — la API de PostgREST no la conoce ni la respeta. Sin RLS, cualquiera con esa key podía pegarle directo a PostgREST y leer/escribir **todas las filas de todas las tablas de todos los workspaces**, sin pasar por ningún chequeo de la app. Se confirmó en vivo antes de corregirlo — ver Verificación.

**Decisión:** nueva migración (`ALTER TABLE ... ENABLE ROW LEVEL SECURITY`) sobre las 14 tablas, **sin agregar ninguna policy**. Es la corrección completa y segura para este caso específico porque la app nunca usa `supabase.from(...)`/PostgREST para datos de negocio (confirmado por grep — el cliente de `@supabase/supabase-js` se usa únicamente para `auth.getUser()`); todo el acceso a datos pasa por Prisma sobre `DATABASE_URL`/`DIRECT_URL`, que se conecta con el rol `postgres` — ese rol tiene `BYPASSRLS` en Supabase, así que queda completamente afuera del efecto de esta migración.

**Alternativas consideradas:**
- Escribir policies granulares por workspace (el patrón "de manual" de Supabase, comparando `auth.uid()` contra `WorkspaceMember`): descartado — tendría sentido si la app leyera datos vía PostgREST con el JWT del usuario, pero acá ninguna lectura pasa por ahí. Policies granulares serían complejidad sin ningún beneficio real, y booleanas más fáciles de dejar mal escritas que un `ENABLE ROW LEVEL SECURITY` sin policies.
- No tocar nada porque "total, la app no usa esa API": descartado de plano — la API sigue estando activa y pública mientras Supabase la sirva, la use la app o no.

**Trade-off:** ninguno funcional — Prisma bypassea RLS por el rol de conexión, así que la app sigue exactamente igual. El único costo es que si en el futuro se quisiera usar PostgREST/`supabase.from()` para algo (client-side realtime, por ejemplo), va a devolver vacío hasta que se agreguen policies explícitas — comportamiento correcto (seguro por defecto), no un bug.
